...
//        if EmojiMemoryGame.oneInit {
        ...
        // userDefaultsManager.storeThemeInUserDefaults(theme: chosenTheme) // 存储主题
//        }
...
struct UserDefaultsManager {
    ...
    private var themeArrayKey: String { "ThemeArray" }
    ...
    // func storeThemeInUserDefaults(theme: ThemeChooser.Theme) {
    //     var themeArray = readThemeArrayFromUserDefaults() ?? []
    //     themeArray.append(theme)
    //     UserDefaults.standard.set(try? PropertyListEncoder().encode(themeArray), forKey: themeArrayKey)
    // }
...
//    func readThemeArrayFromUserDefaults() -> [ThemeChooser.Theme]? {
//        if let data = UserDefaults.standard.value(forKey: themeArrayKey) as? Data,
//           let themeArray = try? PropertyListDecoder().decode([ThemeChooser.Theme].self, from: data)
//        {
//            return themeArray
//        }
//        return nil
...
//
//    func readThemeFromUserDefaults() -> ThemeChooser.Theme? {
//        if let themeArray = readThemeArrayFromUserDefaults(), themeArray.count >= 2 {
//            return themeArray[themeArray.count - 2]
//        }
//        return nil
//    }
    ...
    // 删除数组
    func deleteThemeArrayFromUserDefaults() {
        UserDefaults.standard.removeObject(forKey: themeArrayKey)
    }

    func deleteModelFromUserDefaults() {
